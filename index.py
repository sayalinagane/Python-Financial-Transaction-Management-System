...
from tkinter import messagebox                                   #To display relevant messages depending upon the application requirements.
from time import gmtime, strftime                                # returns the string representation of the date or time object
...
def check_acc_nmb(num):
	try:
		...
		messagebox.showinfo("Error","Invalid Credentials!\nTry Again!")      # while login to check whether the account number is correct or not 
		return 0
	fpin.close()
	return 

def home_return(master):                                          #The destroy() method in Tkinter destroys a widget.when a process is complete by some user action we need
	master.destroy()                                          #to destroy the GUI components to free the memory as well as clear the screen.
	Main_Menu() 

def write(master,name,oc,pin):
	
	if( (is_number(name)) or (is_number(oc)==0) or (is_number(pin)==0)or name==""):
		messagebox.showinfo("Error","Invalid Credentials\nPlease try again.")                 #comparing if the credentials are matching or not 
		master.destroy()
		return 

	f1=open("Accnt_Record.txt",'r')                             #if not matching then create new account                                                   
	...
	frec.write("Date                             Credit      Debit     Balance\n")              # To store record in this manner in file
	frec.write(str(strftime("[%Y-%m-%d] [%H:%M:%S]  ",gmtime()))+"     "+oc+"              "+oc+  "\n")
	frec.close()
	
	messagebox.showinfo("Details","Your Account Number is:"+str(accnt_no))                      #after creating account it will automatically genrate one account number
	master.destroy()
	return
...
def crdt_write(master,amt,accnt,name):
	...
	frec.write(str(strftime("[%Y-%m-%d] [%H:%M:%S]  ",gmtime()))+"     "+str(amti)+"              "+str(cb)+"\n")       # to store the credited amount in file
	frec.close()
	messagebox.showinfo("Operation Successfull!!","Amount Credited Successfully!!")                 #amount credited 
	master.destroy()
	return
...
def debit_write(master,amt,accnt,name):
	...
	if(int(amt)>camt):
		...
		fdet.close()
		frec=open(str(accnt)+"-rec.txt",'a+')
		frec.write(str(strftime("[%Y-%m-%d] [%H:%M:%S]  ",gmtime()))+"     "+"              "+str(amti)+"              "+str(cb)+"\n")    # to store the debited amount in file
		frec.close()
...
def Cr_Amt(accnt,name):
	...
	l_title=tk.Message(creditwn,text="ATM SYSTEM",relief="raised",width=2000,padx=600,pady=0,fg="white",bg="black",justify="center",anchor="center")
...
def De_Amt(accnt,name):
	...
	l_title=tk.Message(debitwn,text="ATM SYSTEM",relief="raised",width=2000,padx=600,pady=0,fg="white",bg="black",justify="center",anchor="center")
	l_title.config(font=("Courier","50","bold"))
	l_title.pack(side="top")
...
def disp_bal(accnt):
	...
	bal=fdet.readline()
	fdet.close()
	messagebox.showinfo("Balance",bal)          #To display reamining balance in account 
...
def disp_tr_hist(accnt):
	...
	l_title=tk.Message(disp_wn,text="ATM SYSTEM",relief="raised",width=2000,padx=600,pady=0,fg="white",bg="black",justify="center",anchor="center")
	l_title.config(font=("Courier","50","bold"))
	l_title.pack(side="top")
...
def logged_in_menu(accnt,name):
	...
	fr1=tk.Frame(rootwn)
	fr1.pack(side="top")
	l_title=tk.Message(rootwn,text="ATM SYSTEM",relief="raised",width=2000,padx=600,pady=0,fg="white",bg="black",justify="center",anchor="center")
	l_title.config(font=("Courier","50","bold"))
	l_title.pack(side="top")
...
def log_in(master):
	...
	l_title=tk.Message(loginwn,text="ATM SYSTEM",relief="raised",width=2000,padx=600,pady=0,fg="white",bg="black",justify="center",anchor="center")
	l_title.config(font=("Courier","50","bold"))
	l_title.pack(side="top")
...
def Create():
	...
	l_title=tk.Message(crwn,text="ATM SYSTEM",relief="raised",width=2000,padx=600,pady=0,fg="white",bg="black",justify="center",anchor="center")
	l_title.config(font=("Courier","50","bold"))
	l_title.pack(side="top")
...
def Main_Menu():
	...
	rootwn.title("ATM System")
	rootwn.configure(background='yellow')
	fr1=tk.Frame(rootwn)
	fr1.pack(side="top")
	bg_image = tk.PhotoImage(file ="pile1.gif")
	l_title=tk.Message(text="ATM SYSTEM",relief="raised",width=2000,padx=600,pady=0,fg="white",bg="black",justify="center",anchor="center")
	l_title.config(font=("Courier","50","bold"))
	l_title.pack(side="top")
